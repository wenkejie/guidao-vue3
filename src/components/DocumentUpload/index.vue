<template>
  <div class="upload-file">
    <el-upload :before-upload="handleBeforeUpload" v-model:file-list="fileList" :limit="limit"
      :on-error="handleUploadError" :on-exceed="handleExceed" :on-success="handleUploadSuccess" :show-file-list="false"
      :data="curUploadData" :drag="drag" :headers="headers" :auto-upload="autoUpload" class="upload-file-uploader"
      ref="documentUpload">
      <!-- 拖拽上传 -->
      <template v-if="drag">
...
import { addDocument } from '@/api/files/document.js'
...
const uploadFileUrl = ref('/api/api/folder/51094603-4038-4ef0-ab7f-26e07433467d') // 上传的文件存储地址
const headers = ref({ Authorization: 'Bearer ' + getToken() })
const fileList = ref([])
const showTip = computed(() => props.isShowTip && (props.fileType || props.fileSize))
const uploadData = computed(() => props.data)

const curUploadData = ref()
...
function handleBeforeUpload(file) {
  ...
  // // 校检文件类型
  // let upGroup = new FormData
  // upGroup.append(_name, file)
  // curUploadData.value = upGroup
  // console.log(curUploadData.value.get(_name), 'uploadData.value')
...
function submitUpload() {
  ...
  fileList.value.forEach(element => {
    ...
    let _id = uploadData.value.physicalFolderId
    _form.append(element.name, element.raw)
    addDocument(_id, _form).then((res) => {
      console.log(res, 'res')
      ElMessage({
        message: '上传成功',
        type: 'success',
    ...
    console.log(element.raw, 'ele')
  });

  // proxy.$refs.documentUpload.submit()
}
defineExpose({
  submitUpload
})
