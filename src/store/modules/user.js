    ...
    getInfo() {
      ...
      return new Promise((resolve, reject) => {
        getInfo(_id)
          .then((res) => {
            const data = res
            const avatar = data.profilePhoto == '' ? defAva : data.profilePhoto

            if (data.userClaims && data.userClaims.length > 0) {
              // 验证返回的roles是否是一个非空数组
              this.roles = data.userClaims
            } else {
            ...
            this.avatar = avatar
            this.userInfo = data //新加
            this.userId = data.id //新加
            this.userName = data.userName //新加
            resolve(res)
          })
          .catch((error) => {
            console.warn(error)
